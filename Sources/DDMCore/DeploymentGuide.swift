...
public enum DeploymentGuide {
    ...
    public static func markdown(report: MigrationReport) -> String {
        ...
        md += "| **Jamf Pro** | ✅ Yes (Blueprints) | UI: **Blueprints → Custom Declarations → Add item** — set Kind=Configuration, Channel, Type = the declaration's `Type`, Payload = its `Payload` object (use the `.payload.json` companion). Jamf generates the Identifier/ServerToken. Or push via API with `ddm-migrate --push-jamf --jamf-url … --jamf-tenant …` (token via `JAMF_API_TOKEN`; `--jamf-include-legacy` sends legacy payloads as a configuration-profile component). |\n"
